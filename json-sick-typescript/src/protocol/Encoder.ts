...
class I16Type implements FixedEncoder<number> {
  ...
  get blobSize() { return I16Type.blobSize }
}
export const Short = (value: number, endian = Endian.Little) => new I16Type(endian).getBytes(value);

class I32Type implements FixedEncoder<number> {
  constructor(private endian = Endian.Little) {}
  static get blobSize() { return 4; }
  getBytes(value: number): Uint8Array {
    const buffer = new ArrayBuffer(4);
    new DataView(buffer).setInt32(0, value, this.endian === Endian.Little);
    return new Uint8Array(buffer);
  }
  get blobSize() { return I32Type.blobSize }
}
export const Int = (value: number, endian = Endian.Little) => new I32Type(endian).getBytes(value);
export const Long = (value: bigint, endian = Endian.Little) => new I64Type(endian).getBytes(value);
...
class F32Type implements FixedEncoder<number> {
  ...
  get blobSize() { return F32Type.blobSize }
}
export const Float = (value: number, endian = Endian.Little) => new F32Type(endian).getBytes(value);
...
export class F64Type implements FixedEncoder<number> {
  ...
  get blobSize() { return F64Type.blobSize }
}
export const Double = (value: number, endian = Endian.Little) => new F64Type(endian).getBytes(value);

export class RefType implements FixedEncoder<SickRef> {
  constructor(private endian = Endian.Little) {}
  static get blobSize() { return 1 + I32Type.blobSize; }
  ...
  get blobSize() { return RefType.blobSize }
}
export const Ref = (ref: SickRef, endian = Endian.Little) => new RefType(endian).getBytes(ref);

export class ObjType implements FixedEncoder<ObjEntry> {
  constructor(private endian = Endian.Little) {}
  static get blobSize() { return 1 + RefType.blobSize; }
  ...
  get blobSize() { return ObjType.blobSize }
}
export const Obj = (ref: ObjEntry, endian = Endian.Little) => new ObjType(endian).getBytes(ref);
export const Root = (root: SickRoot, endian = Endian.Little) => new ObjType(endian).getBytes(root);

export class ArrayByteType<T> implements ArrayEncoder<T> {
  constructor(private encoder: FixedEncoder<T>, private endian = Endian.Little) {}
  getBytes(value: T[]): Uint8Array {
    const countBytes = Int(value.length);
    const elementBytes = flattenU8Array(value.map(e => this.encoder.getBytes(e)));
...
export const RefList = (refs: SickRef[], endian = Endian.Little) => new ArrayByteType(new RefType(endian), endian).getBytes(refs);
export const RootList = (roots: SickRoot[], endian = Endian.Little) => new ArrayByteType(new ObjType(endian), endian).getBytes(roots);
